 ...
 * 리턴값: 매칭 정확도 (0-1)
 */ export function calculateMatchAccuracy(commentText, productTitle, matchedWords) {
  if (!matchedWords || matchedWords.length === 0) return 0;
  // 1. 매칭된 단어들의 총 길이
  const matchedLength = matchedWords.reduce((sum, word)=>sum + word.length, 0);
  // 2. 댓글에서 상품 관련 단어들만 추출 (숫자, 전화번호, 지역명 제외)
  const commentTokens = normalizeAndTokenize(commentText);
  const productTokens = commentTokens.filter((token)=>!/^\d+$/.test(token) && // 숫자만 있는 토큰 제외
    !token.includes("점") && // 지역명 제외
    token.length >= 2 // 2글자 이상만
  );
  const productRelatedLength = productTokens.reduce((sum, token)=>sum + token.length, 0);
  // 3. 상품명 길이
  const productLength = productTitle.replace(/\s+/g, "").length;
  // 4. 정확도 계산
  const commentCoverage = productRelatedLength > 0 ? matchedLength / productRelatedLength : 0;
 ...
 */ export function calculateTextSimilarity(text1, text2) {
  ...
  usageKeywords.forEach((keyword)=>{
    const inText1 = text1Lower.includes(keyword);
    const inText2 = text2Lower.includes(keyword);
    if (inText1 && inText2) {
      // 양쪽 모두에 같은 용도 키워드가 있으면 매칭
      matchedWords.push(keyword);
      usageMatched = true;
    } else if (inText1 || inText2) {
      // 한쪽에만 용도 키워드가 있으면 불일치
      usageMismatch = true;
    }
  });
  // 옵션 키워드 직접 매칭
  ...
  // 🔥 "1박스" ↔ "1박" 유사성 처리
  // 댓글이 "1박스"이고 상품명이 "1박"인 경우 또는 그 반대
  const boxToPackPatterns = [
    {
      comment: "1박스",
      product: "1박"
    },
  ...
  boxToPackPatterns.forEach((pattern)=>{
    if (text1Lower.includes(pattern.comment) && text2Lower.includes(pattern.product) || text1Lower.includes(pattern.product) && text2Lower.includes(pattern.comment)) {
      // 이미 매칭되지 않았다면 추가
      if (!matchedWords.includes(pattern.product) && !matchedWords.includes(pattern.comment)) {
        matchedWords.push(pattern.product);
      }
    }
  });
  // 2차: 복잡한 단어 매칭 (부분 매칭)
  ...
  // 🔥 용도 키워드 가중치 적용한 유사도 계산
  let weightedSimilarity = matchCount / Math.max(totalTokens1, totalTokens2);
  if (usageMatched) {
    // 용도 키워드가 정확히 매칭되면 가중치 부여 (2.0배) - 더 강한 가중치
    weightedSimilarity *= 2.0;
  } else if (usageMismatch) {
    // 용도 키워드가 불일치하면 페널티 (0.2배) - 더 강한 페널티
    weightedSimilarity *= 0.2;
  }
  // 최대 1.0으로 제한
  const similarity = Math.min(weightedSimilarity, 1.0);
  ...
  return {
    ...
    complexSyllables: totalComplexSyllables > 0 ? totalComplexSyllables : undefined
  };
}
/**
 ...
 */ export function findBestProductMatch(commentText, productMap) {
  ...
  productMap.forEach((product, itemNumber)=>{
    ...
    if (similarity.matchCount > 0) {
      candidates.push({
        ...
        matchAccuracy: calculateMatchAccuracy(commentText, cleanTitle, similarity.matchedWords)
      });
      const matchAccuracy = calculateMatchAccuracy(commentText, cleanTitle, similarity.matchedWords);
    } else {
  ...
  candidates.sort((a, b)=>{
    ...
    const aPartialMatches = a.similarity.commentTokens.map((token)=>{
      ...
      return [];
    }).flat().filter((part)=>part.length >= 2);
    const bPartialMatches = b.similarity.commentTokens.map((token)=>{
      const cleanToken = token.replace(/\d+/g, "");
      if (cleanToken.length >= 4) {
        const parts = [];
        for(let i = 0; i <= cleanToken.length - 2; i += 2){
    ...
    if (aHasExactMatch && bHasExactMatch) {
      const aMatchRatio = a.similarity.commentTokens.filter((token)=>{
        const cleanToken = token.replace(/\d+/g, "");
        return cleanToken.length >= 2 && a.productTitle.includes(cleanToken);
      }).reduce((acc, token)=>acc + token.replace(/\d+/g, "").length, 0) / a.productTitle.length;
      const bMatchRatio = b.similarity.commentTokens.filter((token)=>{
        const cleanToken = token.replace(/\d+/g, "");
        return cleanToken.length >= 2 && b.productTitle.includes(cleanToken);
      }).reduce((acc, token)=>acc + token.replace(/\d+/g, "").length, 0) / b.productTitle.length;
    ...
    const aMatchLength = a.similarity.matchedWords.reduce((sum, word)=>sum + word.length, 0);
    const bMatchLength = b.similarity.matchedWords.reduce((sum, word)=>sum + word.length, 0);
  ...
  //   )}]`
  // );
  if (bestMatch.similarity.complexMatches && bestMatch.similarity.complexMatches.length > 0) {
  // console.log(
