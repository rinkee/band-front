/**
 * AI 및 데이터 처리 함수
 * 게시물 내용에서 픽업 날짜를 추출하고 향상시키는 함수
 */

import { convertUTCtoKST, kstDateToUtcISOString, updateTitleWithDate } from '../utils/dateUtils.js';

/**
 * 게시물 내용에서 pickup_date를 추출 및 향상시키는 함수
 *
 * @param {Object|null} aiAnalysisResult - AI 분석 결과 (products 배열 포함)
 * @param {string} postContent - 게시물 내용
 * @param {Object} post - 게시물 정보 (createdAt 포함)
 * @returns {Object} 향상된 AI 분석 결과
 *
 * @description
 * AI 결과가 없어도 기본 구조를 생성하여 후처리를 진행합니다.
 * 게시물 내용에서 다음 정보를 추출합니다:
 * - 시간 패턴 (N시, 오전/오후 N시, N:MM 등)
 * - 날짜 패턴 (N월 N일, 상품수령기간 등)
 * - 요일 패턴 (월요일, 화요일 등, 오늘, 내일, 모레)
 * - 다음주 키워드
 *
 * 추출된 정보를 바탕으로 pickup_date를 계산하고,
 * 영업시간(8시~20시)을 고려하여 시간을 조정합니다.
 */
export function enhancePickupDateFromContent(aiAnalysisResult, postContent, post) {
  // logger는 사용 시 import 필요
  const logger = typeof window !== 'undefined' && window.console ? window.console : console;

  // AI 결과가 없어도 기본 구조 생성하여 후처리 진행
  if (!postContent || !post) {
    logger.warn('[PICKUP_DATE 후처리] 필수 데이터 없음, 원본 반환');
  ...
  if (receiptMatch) {
    ...
    logger.info('[PICKUP_DATE 후처리] 상품수령기간 특수 패턴 감지', {
