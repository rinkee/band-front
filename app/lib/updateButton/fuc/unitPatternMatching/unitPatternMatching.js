...
import { smartUnitMapping } from '../orderPatternExtraction/orderPatternExtraction.js';
...
export function extractOrderByUnitPattern(
  ...
  // 0단계: "k" 단위 처리 - 한글 어미 지원 강화
  const kMatch = text.match(/(\d+)\s*k(?:\s*주문)?\s*[가-힣]*\s*[.,!?~]*\s*$/i);
  if (kMatch && kMatch[1]) {
  ...
  // 1단계: 우선 개/대/요 매칭 - 한글 어미 지원 강화
  const universalMatch = text.match(/(\d+)\s*(?:개|대|요)\s*[가-힣]*\s*[.,!?~]*\s*$/i);
  ...
  // 2단계: 박스 단위 우선 매칭 (제습제 문제 해결)
  const hasBoxUnit = /(박스|박)/.test(text);
  if (hasBoxUnit) {
    ...
    for (const [itemNumber, productInfo] of productMap) {
      const quantityText = productInfo.quantity_text;
      if (quantityText && /박스|박/.test(quantityText)) {
        const similarUnits = smartUnitMapping[quantityText] || [quantityText];

        // 박스 관련 단위에 대해 매칭 시도
        for (const unit of similarUnits) {
          // 박스/박 단위 매칭 패턴
          const boxPattern = new RegExp(`(\\d+)\\s*${unit}\\s*[가-힣]*\\s*[.,!?~]*\\s*$`, "gi");
          const match = text.match(boxPattern);
          ...
          if (match && match[0]) {
            ...
            if (numberMatch && numberMatch[1]) {
              ...
              if (quantity >= 1 && quantity <= 999) {
                console.log(
                  `[박스 단위 매칭 성공] "${commentText}" → 상품 ${itemNumber}번 "${productInfo.title}" (${quantity}${unit})`
                );
                foundOrders.push({
                  itemNumber: itemNumber,
                  quantity: quantity,
                  matchedUnit: unit,
                  actualUnit: quantityText,
                  matchType: "box-unit-priority",
                  isAmbiguous: false,
                  processingMethod: "box-unit-pattern"
                });
                return foundOrders;
              }
            }
  ...
  for (const [itemNumber, productInfo] of productMap) {
    ...
    if (quantityText) {
      ...
      // 각 유사 단위에 대해 매칭 시도
      for (const unit of similarUnits) {
        // 명시적 단위 매칭 패턴 - 한글 어미 지원 강화
        const explicitPattern = new RegExp(`(\\d+)\\s*${unit}\\s*[가-힣]*\\s*[.,!?~]*\\s*$`, "gi");
        ...
        if (match && match[1]) {
          ...
          if (quantity >= 1 && quantity <= 999) {
            foundOrders.push({
              ...
              matchType: pattern === simplePatterns[0] ? "quantity-text-number-only" : "quantity-text-number-with-korean-ending",
              isAmbiguous: false,
              processingMethod: "quantity-text-pattern"
            });
            return foundOrders;
