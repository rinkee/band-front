...
function OrdersTestPageContent({ mode = "raw" }) {
  ...
  const displayOrders = useMemo(() => {
    ...
    // 1. Status 필터링 (주문완료, 수령완료, 주문취소, 결제완료)
    if (filterSelection !== "all" &&
        filterSelection !== "확인필요" &&
        filterSelection !== "미수령" &&
        filterSelection !== "none") {
      // '주문완료+수령가능'인 경우 주문완료만 필터링 (pickup_date는 나중에)
      const targetStatus = filterSelection === "주문완료+수령가능" ? "주문완료" : filterSelection;
      arr = arr.filter(o => {
    ...
    // 2. SubStatus 필터링 (확인필요, none)
    if (filterSelection === "확인필요") {
      arr = arr.filter(o => o.sub_status === "확인필요");
    } else if (filterSelection === "none") {
      arr = arr.filter(o => !o.sub_status || o.sub_status === "none");
    }

    // 3. Pickup date 필터링
    // 수령가능만 보기: pickup_date >= today
    ...
    // 미수령 필터: pickup_date < today AND status가 수령완료/주문취소가 아님
    if (filterSelection === "미수령") {
      arr = arr.filter((o) => {
        // 수령완료, 주문취소 제외
        const status = o.status || o.order_status;
        if (status === "수령완료" || status === "주문취소") {
          return false;
        }
        ...
        const k = new Date(now.getTime() + KST_OFFSET);
        const todayYmd = k.getUTCFullYear() * 10000 + (k.getUTCMonth() + 1) * 100 + k.getUTCDate();
        const ymd = toKstYmdLocal(pickupDate);
        return ymd ? ymd < todayYmd : false;
      });
    }

    // 4. 검색어 필터링 (고객명, 댓글, 상품명, postKey)
    if (searchTerm && searchTerm.trim()) {
      const term = searchTerm.trim().toLowerCase();

      arr = arr.filter((o) => {
        // 고객명 검색
        const customerName = (o.customer_name || o.commenter_name || '').toLowerCase();
        if (customerName.includes(term)) return true;

        // 댓글 검색
        const comment = (o.comment || o.comment_body || '').toLowerCase();
        if (comment.includes(term)) return true;

        // postKey 검색 (정확 매칭)
        const postKey = o.post_key || o.postKey || '';
        if (postKey === searchTerm.trim()) return true;
        ...
        let productList = [];
        if (pk && postProductsByPostKey[pk]) {
          productList = postProductsByPostKey[pk];
        } else if (band != null && postNum != null) {
          const k = `${band}_${String(postNum)}`;
          if (postProductsByBandPost[k]) {
            productList = postProductsByBandPost[k];
          }
        }
        ...
        if (Array.isArray(productList) && productList.length > 0) {
          for (const product of productList) {
            ...
            if (productTitle.includes(term)) return true;
          }
        }
    ...
    return arr;
  }, [orders, showPickupAvailableOnly, filterSelection, postProductsByPostKey, postProductsByBandPost, searchTerm]);
  ...
  const groupedOrders = useMemo(() => {
    // 검색어가 있으면 전체 결과 표시 (페이지네이션 없음)
    if (searchTerm) {
      return allGroupedOrders;
    }
    // 모든 필터에서 클라이언트 측 페이지네이션 사용
    const startIndex = (currentPage - 1) * itemsPerPage;
    const endIndex = startIndex + itemsPerPage;
    return allGroupedOrders.slice(startIndex, endIndex);
  }, [allGroupedOrders, searchTerm, currentPage, itemsPerPage]);
  ...
  // 모든 필터링을 클라이언트에서 처리하기 위해 전체 데이터를 가져옴
  const ordersFilters = {
    // 항상 전체 데이터 가져오기 (클라이언트에서 필터링)
    limit: 10000,
    sortBy,
    sortOrder,
    // 서버 필터는 사용하지 않음 (모든 필터링은 클라이언트에서)
    status: undefined,
    subStatus: undefined,
    // --- 검색은 클라이언트에서만 처리 (서버에는 전달하지 않음) ---
    search: undefined, // 클라이언트에서 searchTerm으로 필터링
  ...
  // 항상 page=1로 고정 (전체 데이터를 한 번에 가져옴)
  ...
  // 마지막으로 로드한 상품 signature 저장 (중복 fetch 방지)
  ...
  useEffect(() => {
    const fetchBatchProducts = async () => {
      try {
        ...
        console.log('✅ [상품] 로드 완료:', Object.keys(byPostKeyMap).length + Object.keys(byBandPostMap).length, '게시물');
        setPostProductsByPostKey(byPostKeyMap);
        setPostProductsByBandPost(byBandPostMap);

        // sessionStorage에 저장 (페이지 재진입 시 즉시 복원)
        try {
    ...
    fetchBatchProducts();
  }, [userData?.userId, productKeysSignature]); // productKeysSignature가 변경될 때만 실행
  ...
  const {
    ...
    userData?.userId,
    {
      ...
      subStatus: (() => {
        ...
        // 미수령은 클라이언트에서만 필터링 (서버 필터 적용 안 함)
        if (filterSelection === "미수령") {
          return undefined;
        }
        if (
  ...
  const handleCellClickToSearch = useCallback((searchValue, postKey = null) => {
    if (!searchValue && !postKey) return; // 빈 값은 무시
  ...
  const paginate = useCallback((pageNumber) => {
    // 모든 필터에서 클라이언트 측 totalPages 사용
    const total = Math.ceil(allGroupedOrders.length / itemsPerPage);
  ...
  // 모든 필터에서 클라이언트 측 페이지네이션 사용
  // 검색 시 전체 결과 표시 (페이지네이션 없음)
  const totalItems = allGroupedOrders.length;
  const totalPages = Math.ceil(allGroupedOrders.length / itemsPerPage);
  ...
  return (
    <div className="h-full bg-gray-100 text-gray-900 flex overflow-hidden">
      ...
      <aside
        ...
        ${isSidebarCollapsed ? "lg:-translate-x-full lg:w-0" : "lg:translate-x-0 lg:w-64"}
        fixed lg:relative
        w-64 bg-white
                  ...
                  {isStatusFilterOpen && (
                    <div className="border-t border-gray-200 bg-gray-50">
                      {orderStatusOptions.map((option) => {
                        ...
                        const showCount = option.value === "미수령" && unreceivedCount > 0;
                        return (
                          <button
                            key={option.value}
                            onClick={() => {
      ...
      {/* 데스크탑 사이드바 열기 버튼 - 사이드바가 닫혔을 때만 표시 */}
      {isSidebarCollapsed && (
        <button
          onClick={() => setIsSidebarCollapsed(false)}
          className="hidden lg:flex fixed top-18 left-4 z-30 items-center gap-2 px-3 py-2 bg-white rounded-lg shadow-md hover:shadow-lg transition-all duration-300"
          title="패널 열기"
        >
          <ChevronRightIcon className="w-5 h-5 text-gray-600" />
          <span className="text-sm font-medium text-gray-700">패널 열기</span>
        </button>
      )}
              ...
              <div className="flex flex-wrap gap-3 items-center justify-between">
                ...
                <div className="flex items-center gap-6 flex-shrink-0">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      <CurrencyDollarIcon className="w-4 h-4 inline-block mr-1" />
                      일괄결제
                    </button>
                    <button
                    ...
                    >
                      <ArrowLongLeftIcon className="w-4 h-4 inline-block mr-1" />
                      주문으로 되돌리기
                    </button>
                    <button
              ...
              <table className="min-w-full ">
                <thead className="bg-black sticky top-0 z-10">
                  <tr>
                    ...
                    <th className="py-2 px-2 text-left text-sm font-semibold text-gray-600 uppercase tracking-wider w-24 bg-gray-50">
                      <button
                    ...
                    <th className="py-2 px-4 text-center text-sm font-semibold text-gray-600 uppercase tracking-wider w-24 bg-gray-50">
                      상태
                    </th>
                    <th className="py-2 px-4 text-center text-sm font-semibold text-gray-600 uppercase tracking-wider w-28 bg-gray-50">
                      수령일시
                    </th>
                    ...
                    <th className="py-2 px-4 text-center text-sm font-semibold text-gray-600 uppercase tracking-wider w-40 bg-gray-50">
                      바코드
                    </th>
                    <th className="py-2 px-2 text-center text-sm font-semibold text-gray-600 uppercase tracking-wider w-32 bg-gray-50">
                      <button
                    ...
                    return (
                      <React.Fragment key={group.groupId}>
                        ...
                        >
                          ...
                          {/* 고객명 */}
                          <td
                            className="py-2 pr-2 text-base text-gray-700 whitespace-nowrap w-24 truncate hover:text-orange-600 hover:underline cursor-pointer"
                          ...
                          {/* 수령일시 */}
                          <td className="py-2 px-4 text-center text-base text-gray-700 w-28">
                            {(() => {
                          ...
                          <td className="py-2 px-4 text-base text-gray-600">
                            {(() => {
                              ...
                              if (!commentChangeData) {
                                return (
                                  <div className="line-clamp-3 break-words leading-tight font-semibold" title={currentComment}>
                                    {currentComment || "-"}
                                  </div>
                                );
                              }
                          ...
                          <td
                            className="py-2 pl-4 text-base text-gray-700 align-top"
                            onClick={(e) => e.stopPropagation()}
                          >
                            {(() => {
                              const list = getCandidateProductsForOrder(order) || [];
                                    ...
                                    return (
                                      ...
                                      >
                                        ...
                                        <div className="flex-1">
                                          <div className="flex items-center gap-2">
                                            {list.length > 1 && (
                                              <span className="inline-flex h-8 w-8 items-center justify-center rounded-md border border-gray-300 bg-white text-[13px] font-semibold text-gray-900">
                                                {itemNo}번
                                              </span>
                                            )}
                                            <span
                                              className={`text-base leading-snug text-gray-900 font-medium whitespace-nowrap cursor-pointer hover:text-orange-600 hover:underline`}
                                          ...
                                          {price != null && (
                                            <div className="text-base text-gray-700 mt-0.5">₩{price.toLocaleString()}</div>
                                          )}
                                        </div>
                                      </div>
                                    );
                          ...
                          {/* 바코드 */}
                          <td className="py-2 pr-4 text-center text-base text-gray-700 w-32 align-top">
                            {(() => {
                              const list = getCandidateProductsForOrder(order) || [];
                          ...
                          {/* 주문일시 */}
                          <td className="py-2 px-2 text-center text-base text-gray-600 whitespace-nowrap w-32">
