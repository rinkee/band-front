...
export async function GET(request) {
  try {
    ...
    // 2) Band Open API 호출 (게시물 목록)
    const bandApiUrl = `https://openapi.band.us/v2/band/posts?access_token=${user.band_access_token}&band_key=${bandKey}&locale=ko_KR`;
    ...
    if (!response.ok) {
      ...
      console.error("[Band Posts API 에러]", response.status, errorText);
      return NextResponse.json(
        {
    ...
    console.error("[Band Posts API 처리 오류]", error);
    return NextResponse.json(
      {
        error: "서버 내부 오류",
